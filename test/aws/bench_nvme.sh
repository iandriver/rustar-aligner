...
# --- Mount the instance-store NVMe at /root so ALL build/index/ref/fastq/output I/O
# is on fast LOCAL NVMe instead of gp3 EBS. This removes the EBS index-reload variance
# that made STARsolo's wall time swing 76-261 s across runs. (m7id.4xlarge has a
# ~950 GB instance-store NVMe; the EBS root just holds the OS.) Everything below uses
# /root paths, so mounting here redirects all of it to NVMe with no other changes.
...
df -h /root  # confirm /root is the ~950 GB NVMe, not EBS
